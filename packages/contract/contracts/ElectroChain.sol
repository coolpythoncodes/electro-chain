...
contract ElectroChain is MACI {
    struct PollData {
        uint256 id;
        string name;
        bytes encodedOptions;
        string metadata;
        ElectroChain.PollContracts pollContracts;
        uint256 startTime;
        uint256 endTime;
    ...
    mapping(address => PubKey) public addressToPubKey;
    event PollCreated(
    ...
    function signUp(
        ...
        addressToPubKey[msg.sender] = PubKey({x: _pubKey.x, y: _pubKey.y});
    }
